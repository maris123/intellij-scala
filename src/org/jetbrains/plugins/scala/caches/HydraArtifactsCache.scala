...
object HydraArtifactsCache {
  ...
  def getOrDownload(scalaVersion: String, hydraVersion: String, listener: (String) => Unit): Unit = {
    val artifacts = hydraGlobalSettings.getArtifactsFor(scalaVersion, hydraVersion)
    ...
    if (checkIfArtifactsExist(paths)) {
      ...
      maybeHydraBridge match {
        ...
        case Some(hydraBridge) =>
          ...
          hydraGlobalSettings.putArtifactsFor(scalaVersion, hydraVersion, artifactPaths)
      }
    } else {
