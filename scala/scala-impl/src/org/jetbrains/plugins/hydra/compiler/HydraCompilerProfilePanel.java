...
public class HydraCompilerProfilePanel extends JPanel {
  private final HydraCompilerSettingsProfile myDefaultProfile = new HydraCompilerSettingsProfile("Default");
  private final List<HydraCompilerSettingsProfile> myModuleProfiles = new ArrayList<>();
  private final Map<String, Module> myAllModulesMap = new HashMap<>();
  private final Project myProject;
  private final Tree myTree;
  private final ScalaHydraCompilerSettingsPanel mySettingsPanel;
  private HydraCompilerSettingsProfile mySelectedProfile = null;

  public HydraCompilerProfilePanel(Project project) {
    ...
    final JPanel treePanel =
      ToolbarDecorator.createDecorator(myTree).addExtraAction(new AnActionButton("Move to", AllIcons.Actions.Nextfile) {
        ...
        public void actionPerformed(AnActionEvent e) {
          ...
          final HydraCompilerSettingsProfile nodeProfile = ((ProfileNode)node.getParent()).myProfile;
          final List<HydraCompilerSettingsProfile> profiles = new ArrayList<>();
          profiles.add(myDefaultProfile);
          for (HydraCompilerSettingsProfile profile : myModuleProfiles) {
          ...
          final JBPopup popup = JBPopupFactory.getInstance().createListPopupBuilder(list)
            ...
            .setItemChoosenCallback(new Runnable() {
              ...
              public void run() {
                ...
                if (value instanceof HydraCompilerSettingsProfile) {
                  final HydraCompilerSettingsProfile chosenProfile = (HydraCompilerSettingsProfile)value;
      ...
      public void valueChanged(TreeSelectionEvent e) {
        ...
        if (path != null) {
          ...
          if (node instanceof ProfileNode) {
            final HydraCompilerSettingsProfile nodeProfile = ((ProfileNode)node).myProfile;
            final HydraCompilerSettingsProfile selectedProfile = mySelectedProfile;
    ...
    mySettingsPanel = new ScalaHydraCompilerSettingsPanel(myProject);
  ...
  public void initProfiles(HydraCompilerSettingsProfile defaultProfile, Collection<HydraCompilerSettingsProfile> moduleProfiles) {
    myDefaultProfile.initFrom(defaultProfile);
    myModuleProfiles.clear();
    for (HydraCompilerSettingsProfile profile : moduleProfiles) {
      HydraCompilerSettingsProfile copy = new HydraCompilerSettingsProfile("");
  ...
  public HydraCompilerSettingsProfile getDefaultProfile() {
    final HydraCompilerSettingsProfile selectedProfile = mySelectedProfile;
  ...
  public List<HydraCompilerSettingsProfile> getModuleProfiles() {
    final HydraCompilerSettingsProfile selectedProfile = mySelectedProfile;
    if (myDefaultProfile != selectedProfile) {
      mySettingsPanel.saveTo(selectedProfile);
    }
  ...
  private class MyTreeModel extends DefaultTreeModel implements EditableTreeModel {
    ...
    public TreePath addNode(TreePath parentOrNeighbour) {
      ...
      if (newProfileName != null) {
        final HydraCompilerSettingsProfile profile = new HydraCompilerSettingsProfile(newProfileName);
  ...
  private class ProfileNode extends DefaultMutableTreeNode implements DataSynchronizable {
    private final HydraCompilerSettingsProfile myProfile;
    private final boolean myIsDefault;

    public ProfileNode(HydraCompilerSettingsProfile profile, RootNode parent, boolean isDefault) {
      super(profile);
      setParent(parent);
